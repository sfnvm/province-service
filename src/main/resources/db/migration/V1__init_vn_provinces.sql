-- Vietnam administrative units (post-2025 2-tier): schema + full seed.
-- Copied verbatim from vietnamese-provinces-database/postgresql/
--   (postgres_CreateTables_vn_units.sql + postgres_ImportData_vn_units.sql).
-- Kept as a single V1 on purpose: the dev DB is already baselined at v1, so
-- Flyway skips this there; it only runs on fresh DBs (local, prod).
